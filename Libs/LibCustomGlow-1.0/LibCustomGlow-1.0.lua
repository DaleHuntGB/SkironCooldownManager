...
local SCMlc1 = function(progress,s,th,p)
    local c
...
local SCMlc2 = function(progress,s,th,p)
    local c
...
local  pUpdate = function(self,elapsed)
    ...
    if self:IsShown() then
        ...
        for k,line  in pairs(self.textures) do
            line:SetPoint("TOPLEFT",self,"TOPLEFT",SCMlc1((progress+self.info.step*(k-1))%1,width,self.info.th,self.info.pTLx),-SCMlc2((progress+self.info.step*(k-1))%1,height,self.info.th,self.info.pTLy))
            line:SetPoint("BOTTOMRIGHT",self,"TOPLEFT",self.info.th+SCMlc2((progress+self.info.step*(k-1))%1,width,self.info.th,self.info.pBRx),-height+SCMlc1((progress+self.info.step*(k-1))%1,height,self.info.th,self.info.pBRy))
        end
    end
end
